...
>dissolved O2, nutrients- N/P, mircobes, odor/color, invasive species, contanminents
## Session Set Up
...
# epi is the surface of the lake, nutrients at this level
...
> combined index of nurtrient levels and clarity or turbidity 
> amount of biomass a system can support, this is a tough number to get
> look for changes in trophic state overtime can indicate x changes
...
> number coorlates to specific state, allows us to quantify trophic state
...
> not an actual condition but its a good way to measure adn quantify clarity
...
# print(tpbymonth)
# what is phosphorus made of? phosphate- dissolved, some not completely, phosphorus
```
...
>More chlorophil is less secchi depth... but from these graphs, we can just see that more lakes are measured in the summer, vs the winter
>secchi depth is also full of human error, but its so easy and a good measure
...
#   #what information goes into function metrics? dimension (-1, (18 states) 19), 0&40 aligatrophic states
...
#   # represents different trophic states
...
```{r}
write.csv(./Data/LAGOStropic.csv, raw.names = FALSE)
```
