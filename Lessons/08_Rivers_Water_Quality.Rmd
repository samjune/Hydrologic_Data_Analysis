...
> Stands for a bunch of different water parameters.
> Species that indicate all is not well.
> Presense and absence of certain bacteria.
...
> It is sometimes hard to know whether or not they are there. There are tests to see if they are there.
...
>> measure how many cells are in one bacteria
...
> >spread water on pitri dish, heat up and see what grows
...
> If using only one way, may be errors. Want to use cross-validation.

Where and when do you think the biggest risks from harmful bacteria are?
> Take more time and effort. What is the frequency of these water samples?
> Really only going to sample in areas where we think there are going to be issues. Has to be a high priority area, creates bias. Drinking water reseviors get sampled more. Downstream from water treatment plants also get sampled a lot.
...
>> post industrial cities, cities with old structure, retrofitted
>> point-source discharge, pipes that go straight out into stream
...
dat.raw <- readNWISqw(siteNumbers = "01096550", #Merrimack River upstream of Lowell 
                     parameterCd = c("95200",  #Total cell count
                                     ...
                                     # difference is the fecal size. bigger filter passes more things through, more water passes through and more colonies grow with that water.
                                     "31616",  #Fecal coliforms (0.45 um filter)
                                     # what difference does this make in results? difference in filter size. smaller filter going to capture more.
                                     "31673",  #Fecal streptocci KF streptocaccus
                                     # different subsections of genus
...
Microbes.plot <- plot_grid(Total.coli.plot +
                           ...
                           ncol = 1)
# took each plot and made y-axis plank
# legend postion option- move legend to top
# change size of title in theme. theme(plot.title = element_text(size = 3))
# something like that! Google it. Use help functions. ?theme()
...
> scales on graph are different, none of the data is continuous, all different measures
> Total coliform is everything, fecal coliform is subset, next broadest is .7 (lets more things through because it is a larger filter size), most restrictive is .45
>Total coliform would give the largest number- more colonies
> just because data isnt consistent with itself doesnt mean you have to through it out
...
> water quality for communities that have to drink water downstream
...
> Highest in summer because grow more in heat

Does this pattern also exist for the other microbial indicators?

> not all coliform are fecal coliform
...
> water comes from municipal, sewage and rain water drain, combine at water treatment plant
...
# scale on different y axis, each has their own
  dyAxis(name = "y", label = "Discharge (cfs)") %>%
  dyAxis(name = "y2", label = "Total Coliform (cfu per 100 ml)", valueRange = c(0, 600000)) %>%
  dyRangeSelector()
# specify the range 0 to 600,000
# might have to do something different in the homework

# visualize with dygraph

# dygraphs makes the function, have to send it time series data
```
...
> Looking for where there is a lot of microbes around a peak discharge
