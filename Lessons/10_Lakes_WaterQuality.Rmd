...
> Variable 1: chla- chlorophyll a (µg/l)

> Variable 2: dkn- nitrogen, dissolved Kjeldahl (µg/l as N)

> Variable 3: doc- carbon, dissolved organic (µg/l as C)

> Variable 4: tp- phosphorus, total (µg/l as P)

> Variable 5: no2no3- nitrogen, nitrite NO2 + nitrate NO3 nitrogen (µg/l as N)
...
```{r}
# Wrangle data here.
NewData <- LAGOSnutrient %>%
  select(lagoslakeid, sampledate, chla, tn, tp, nh4, no2no3)

NewDataDropNa <- drop_na(NewData)
```
...
```{r}
NewData.correlation <- cor(NewDataDropNa[ ,3:7])
corrplot(NewData.correlation, method = "ellipse")

print(NewData.correlation)
```

What patterns do you see?

> tn is strongly correlated with no2no3, mediumly correlated with tp and mildly correlated with chal and nh4
> tp is strongly correlated with chla, mediumly correlated with tn, mildly correlated with nh4 and not correlated with no2no3.

2. Create two linear regression models (hint: function `lm`) to predict TN and TP using your five predictor variables. 

```{r}
chla_tpTest <- lm(chla ~ tn,NewDataDropNa)
summary(chla_tpTest)
```
