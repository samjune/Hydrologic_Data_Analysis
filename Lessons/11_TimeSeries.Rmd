...
> if we can model the past, we can accurately predict the future
...
> can also be subdaily, not just seasons of the year, can be any scheduled time series
...
> what patterns do we see?
  cyclacle- peaks very 15-20 years, sucra-seasonal, like el nino/la nina, larger than seasonal time sales
  seasonal cycle- 
...
> no. because it does not predict discharge
...
> if you are compairing one day over several years, smaller timescale that elinimates seasonality?
> CHECK IF I AM UNDERSTANDIGN THIS CORRECTLY
...
> snowpacked dominated system in Colorado
...
> anything that is not predicted? CHECK
...
```{r}
ClearCreek_ts <- ts(ClearCreekDischarge[[4]], frequency = 365) # is a value set, not a data frame ?
# table(diff(ClearCreekDischarge$Date))
```
> ts is time series, can only include one column of data, we are not including date
 how do you have time if you do not have date? make sure step is same each time. no NAs in data.
...
> might end up with numbers that ar enot realsitic, the end result sneeds to be real numbers?
