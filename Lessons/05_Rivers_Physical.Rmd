...
>variables discussed: discharge (cubic feet per second, a volume over a given amount of time, relies on cross-section horizontal) and flow (flow velocity, a horizontal distance overtime), bed material- may lead to erosion, mixing- more mixing when the water is moving- turbulent + lamninar flow, slope. 
...
>How do we measure flow? Use a flow meter. Take measurements at different cross-sections. Do that at different areas. Multiply and add different areas. 
If we know a point at a specific stream stage, we can predict what it would be at any stage.
...
>Logrithmic relationship, as discharge increases, gaining less height
>As get more water moving through, height is going to increase less
>Each day the gage will measure height, then will extrapolate

>How change ggplot code?
  switch discharge to y and gage to x
...
>includes a lot of functions you could use in the retrieval package
...
>different places in the US, illustrate differences in regions
...
>Hard to read because lots of codes and abreviation
>Each row is a different type of data, either different variable or statistic
>Sites do not have same types of data collected at each site

What is the period of record for mean daily discharge at each of these sites?
>All the way to the right, have beginning date and end date
...
```
> would leave start and end dates blank if want to chose the earliest and latest
>compare size of data frames, we have data for 10 years we should have ~4,000 variables
>discharge is in feet cube per second, would have to convert for research goals
...
CombinedPlot <- 
  plot_grid(BitterrootPlot, SaukPlot, NehalemPlot, EnoPlot, VerdePlot,
            ncol = 1)
print(CombinedPlot)
```
...
> The Bitterroot and Sauk rivers have a higher magnitude of discharge.
...
> The Bitterroot has a more consistent seasonal cycle, absence of rain in the summer. In Sauk River, there is a snowmelt peak and a wetter summer, the cycle seems to be getting less seasonal in the later years. The Nehalem River seems to peak in the summer. The Eno River seems to have flashier events, the past year was really wet. The Verde River is usually dry but was unsusually wet the past year, there was a huge spike. 

>Interannual: The seasonal cycle is become less defined in the Sauk River. Warming climate, warming winters. More eratic winters. Typical seasonal cycles 
...
>discharge by date then facet by site number
>'rbind' have to have same column titles
...
> Patterns in a given regional geographic area...
> Precipitation vs. discharge, snowmelt vs raion
> Air temperature vs. discharge
