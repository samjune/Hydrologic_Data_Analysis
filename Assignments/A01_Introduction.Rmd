...
>  Link: https://github.com/samjune/Hydrologic_Data_Analysis
...
> ANSWER: How climate change affects water scarcity in the southwest.
>Applying Climate Change Risk Management Tools to Integrate Streamflow Projections
and Social Vulnerability
>link: https://link.springer.com/article/10.1007/s10021-019-00387-5
...
colnames(EnoDischarge)[colnames(MysterySiteDischarge)==
                                 ""]<-"Discharge"
colnames(EnoDischarge)[colnames(MysterySiteDischarge)==
                                 ""]<-"GageHeight"
```
...
```{r}
EnoDischarge$Date <- format(as.Date(EnoDischarge$Date, format="%d/%m/%Y"),"%Y")
```
...
```{r}
Discharge.vs.GageHeight <-
ggplot(EnoDischarge, aes(x = GageHeight, y = Discharge))
