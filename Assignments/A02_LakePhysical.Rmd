...
```{r}
NTL_LTER_Lake_ChemistryPhysics_Raw <- NTL_LTER_Lake_ChemistryPhysics_Raw %>%
  filter(lakename %in% ("Peter Lake"))

NTL_LTER_Lake_ChemistryPhysics_Raw <- filter(Peterdata, year4 == 1984 | year4 == 2016)
```
...
```{r}
Peterdata <- filter(NTL_LTER_Lake_ChemistryPhysics_Raw, lakename == "Peter Lake")
...
```{r}
Julydata <-- separate(NTL_LTER_Lake_ChemistryPhysics_Raw, 
                      sampledate ("y","m","d"))

Julydata <-- separate(NTL_LTER_Lake_ChemistryPhysics_Raw, sampledate, into, sep = "y","m","d", remove = TRUE,
