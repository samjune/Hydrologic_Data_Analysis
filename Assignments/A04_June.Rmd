...
```{r read_data}
dat.02 <- readNWISqw(siteNumbers = "02097314", 
                     parameterCd = c("00300"),
                     startDate = "",
                     endDate = "")
dat.02 <- dat.02 %>%
select(Date = sample_dt,
       DissolvedOxygen = result_va)
...
```{r}
OxygenConcentration.plot <- ggplot(dat.02, aes(x = Date, y = DissolvedOxygen)) +
  geom_point() +
  geom_hline(yintercept = 2)

print(OxygenConcentration.plot)
```

6. What do you notice about the frequency of hypoxia overtime?

> The frequency of hypoxia is decreasing over time. There is less variability in dissolved oxygen levels in 2019 compared to before 1990. Before 1990, hypoxia was more present because levels dissolved oxygen levels dipped below 2mg/L. After 1990, the frequency of dissolved oxygen levels settle around 6 mg/L.
...
nitrogen <- filter(nutrients.dat, Parameter == "00600")
phosporus <- filter(nutrients.dat, Parameter == "00665")
